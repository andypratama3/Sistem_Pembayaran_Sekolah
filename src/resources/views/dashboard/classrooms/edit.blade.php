...
@section('page-header')
    <x-page-header title="Edit Informasi Kelas">
        <x-slot:left>
            {{ Breadcrumbs::render('dashboard.classrooms.edit', $classroom) }}
        </x-slot:left>
        <x-slot:actions>
    ...
    <div class="col-12">
        <form action="{{ route('dashboard.classrooms.update', $classroom) }}" method="POST">
            ...
            @include('dashboard.classrooms.form', [
                ...
                'submitLabel' => 'Update Kelas',
                'formTitle' => 'Edit Data Kelas: ' . $classroom->name,
            ])
        </form>
    </div>
@endsection
